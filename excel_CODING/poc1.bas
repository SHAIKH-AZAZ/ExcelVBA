Sub PlaceShapeFromCode()
    ...
    ' Get active row of selection (ex: if you click H5 → row=5)
    r = ActiveCell.Row
    
    ' 1) Read shape code from column D
    shpCode = ws.Cells(r, "D").Value
    
    If shpCode = "" Then
        MsgBox "Shape code missing in column D" & r, vbExclamation
        Exit Sub
    End If
    ...
    Set targetCell = ws.Cells(r, "H")
    
    newShp.Top = targetCell.Top
    newShp.Left = targetCell.Left
    
    ' 6) Read variable values
    Dim Avalue As String, Bvalue As String, Cvalue As String
    Avalue = ws.Cells(r, "E").Value
    Bvalue = ws.Cells(r, "F").Value
    Cvalue = ws.Cells(r, "G").Value
    ...
    MsgBox "Shape placed at H" & r & " successfully!", vbInformation
